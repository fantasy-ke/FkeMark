...
- **CI/CD**：GitHub Actions；`code-review.yml` 在推送 `dev` 时把本次 Git diff 发送到环境变量配置的 OpenAI-compatible Chat Completions 接口，并将 AI 审核报告写入 Actions Summary、构件和提交评论；`dev.yml` + `release.yml` 负责三阶段发布流水线（release → build → publish）
...
- 标签缓存按统一斜杠和不区分大小写的文件路径匹配；未打开或没有缓存的文件继续通过文件系统读取，保持文件系统优先架构和现有保存语义。
- 回归测试位于 `tests/backlinks.panel.test.tsx`，应覆盖磁盘内容过期但标签缓存已有 `[[目标笔记]]` 时仍能构建反向链接。

## 工具栏配置界面布局约定（2026-07-24）
- 工具栏按钮配置使用单一连续列表，不为每个按钮重复使用独立卡片和相同说明；桌面采用“按钮 / 放置位置 / 前置分隔符”三列布局。
- 720px 以下切换为两列并将分隔符控制放到第二列下一行，520px 以下切换为单列；隐藏项只弱化按钮标识，不改变现有配置模型和行为。

## AI 对话侧栏焦点约定（2026-07-24）
