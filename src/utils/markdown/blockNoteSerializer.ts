...
export interface DirectMarkdownCapableSerializer {
  ...
  __fkeMarkDirectMarkdownCache?: WeakMap<object, Map<string, string>>
  __fkeMarkLastDirectMarkdownMetrics?: BlockNoteDirectMarkdownMetrics
}
...
function linkMarkdown(item: InlineItem): string {
  ...
  return wikiTarget ? `[[${wikiTarget}]]` : `[${content}](${escapeLinkTarget(href)})`
}
...
export function installBlockNoteDirectMarkdown(editor: DirectMarkdownCapableSerializer): void {
  ...
  editor.__fkeMarkDirectMarkdownCache = cache
  editor.blocksToMarkdownDirect = (blocks: unknown[]) => {
    const result = blocksToMarkdownDirect(blocks, cache)
    editor.__fkeMarkLastDirectMarkdownMetrics = result.metrics
    return result
  }
}
