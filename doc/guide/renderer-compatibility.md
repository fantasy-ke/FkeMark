...
7. 标准块直接序列化并使用块对象缓存；遇到未知块时才回退到 BlockNote 的有损 Markdown 序列化。
...
- 无序列表、有序列表、任务列表、引用、代码块、表格、分隔线和基础图片。
- YAML Front Matter 原样保留。
- Wiki 链接继续以 `[[target]]` 形式往返保存。
...
- 图片宽高注释、拖拽缩放、图片编辑菜单、上传占位进度和取消按钮尚未迁移；基础图片插入和上传后的图片块可用。
- 罗马数字、字母等自定义有序列表外观会规范为标准数字有序列表。
- 旧版依赖 TipTap 2 自定义节点的部分悬浮语法提示、代码语言浮层和右键菜单不能保证完全可用。

这些能力需要分别实现 BlockNote 自定义 block、inline content 或 style schema，不能继续复用旧 TipTap 2 节点，否则会重新引入双编辑器状态和同步转换卡顿。
