...
- 代码注释仅用于解释必要的复杂逻辑或平台限制；所有新增或修改的代码注释必须使用中文（第三方协议字段、标准名称和不可翻译的标识符除外），并保持与所在文件风格一致。

## 4. 文件组织与 1200 行限制

- 不得让代码文件超过 1200 行。代码文件包括 TS、TSX、JS、JSX、MTS、CTS、RS、CSS 和 SCSS。
- 修改接近上限的文件时，应提前拆出本次涉及的功能，避免刚好卡在 1200 行附近。
...
- 默认在任务完成并通过必要验证后创建 Git 提交；只有用户明确要求“不提交”“先不要提交”或“只查看”时，才不得暂存或提交。
- 提交前再次核对 `git status`，仅暂存本次任务的明确文件；避免使用 `git add .`。
- 不得把用户原有的未提交修改、构建产物、临时截图或调试文件混入提交。
- 提交信息遵循 Conventional Commits：类型前缀统一使用英文；除类型前缀、代码标识符和不可翻译的专有名称外，提交标题和正文必须使用中文，不得使用英文描述。常用类型包括：
  - `feat:`：新增功能
  - `fix:`：修复问题
  - `docs:`：文档修改
  - `refactor:`：不改变外部行为的重构
  ...
  - `chore:`：其他工程或维护性修改
  - `revert:`：回滚已有提交
- 提交标题格式为 `<type>: <中文描述>`，例如 `feat: 新增模型选择器`、`fix: 修复重复标签页`、`docs: 更新 Git 提交规范`。正文使用中文详细说明：
  - 实现了什么
  - 关键兼容或安全处理
  - 测试与验证结果
