...
- 行数采用两级标准：**硬上限 1200 行**，**完成标准 800 行**。代码文件包括 TS、TSX、JS、JSX、MTS、CTS、RS、CSS 和 SCSS。
- 任何情况下都不得让代码文件超过 1200 行；任务交付时要求所有活跃代码文件不超过 800 行。
- 修改接近 800 行的文件时，应提前拆出本次涉及的功能，避免刚好卡在完成标准附近。
...
- 完成前扫描活跃代码文件行数，确认所有文件不超过 800 行，且没有任何文件触及 1200 行硬上限。
...
- 所有代码文件不超过 800 行，且没有任何文件超过 1200 行硬上限。
- 没有覆盖、暂存或提交无关修改。
- 项目记忆已按任务规模更新。
