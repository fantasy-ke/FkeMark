...
describe('FkeMark MCP npm package', () => {
  ...
  it('publishes and archives the CLI package from a dedicated workflow', () => {
    ...
    expect(parsed.permissions).toEqual({ contents: 'read', actions: 'write', 'id-token': 'write' })
    ...
    expect(publishStep.run).toContain('npm publish --access public')
    expect(publishStep.run).toContain('NPM_CONFIG_PROVENANCE=false')
    expect(publishStep.run).toContain('EPUBLISHCONFLICT')
    expect(publishStep.run).toContain('ENEEDAUTH')
    expect(publishStep.run).toContain('::warning::')
    expect(publishStep.run).toContain('::error::')
